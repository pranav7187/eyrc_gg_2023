...
# Team ID:			GG_2787
# Author List:		Pranav Bakshi, Subarno Maji, Shahzeb Arshad, Shobhit Kumar Shukla
# Filename:			task_2a.py
...
def detect_ArUco_details(image):
    ...
    * ArUco_corners = 
    ...
    for i in range(len(ids)):
        a = corners[i]
        ArUco_corners.update({int(ids[i][0]):a[0]})
        ...
        data_list = [[int(center_x), int(center_y)],angle]
        ArUco_details_dict.update({int(ids[i][0]):data_list})
