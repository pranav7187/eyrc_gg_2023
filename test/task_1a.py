...
class model(nn.Module):
	...
	def __init__(self):
		...
		self.layer_5 = nn.Linear(in_features=64, out_features=64)
		self.layer_6 = nn.Linear(in_features=64, out_features=32)
		self.layer_7 = nn.Linear(in_features=32, out_features=1)
...
def training_function(model, number_of_epochs, tensors_and_iterable_training_data, loss_function, optimizer):
	...
	with torch.inference_mode():
		train_logits = trained_model(X_train).squeeze()
		trian_pred = torch.round(train_logits)
	
		

		correct = 0
		for i in range(len(trian_pred)):
			if trian_pred[i] < 0.5:
				trian_pred[i] = 0
			elif trian_pred[i] >= 0.5:
				trian_pred[i] = 1
			if trian_pred[i] == tensors_and_iterable_training_data[2][i]:
				correct += 1
  	
		model_accuracy_train = correct / len(trian_pred) * 100

		print(f"train acc : {model_accuracy_train}")
